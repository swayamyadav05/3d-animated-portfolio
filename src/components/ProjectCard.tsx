import React, { forwardRef } from "react";

interface Project {
  id: number;
  title: string;
  category: string;
  featured: boolean;
  description: string;
  longDescription: string;
  imgPath: string;
  liveLink: string | null;
  githubLink: string;
  technologies: string[];
  highlights: string[];
  impact: string;
}

interface ProjectCardProps {
  project: Project;
  variant: "main" | "side";
  className?: string;
}

const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ project, variant, className = "" }, ref) => {
    const handleLiveClick = () => {
      if (project.liveLink) {
        window.open(
          project.liveLink,
          "_blank",
          "noopener,noreferrer"
        );
      }
    };

    const handleGithubClick = () => {
      window.open(
        project.githubLink,
        "_blank",
        "noopener,noreferrer"
      );
    };

    if (variant === "main") {
      return (
        <div
          ref={ref}
          className={`first-project-wrapper ${className}`}>
          <div className="image-wrapper group">
            <img
              src={project.imgPath}
              alt={project.title}
              className="transition-all duration-300 group-hover:brightness-90"
            />

            {/* Overlay with links */}
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
              {project.liveLink && (
                <button
                  onClick={handleLiveClick}
                  className="px-6 py-3 bg-white text-black font-semibold rounded-lg transition-all duration-300 hover:bg-white-50 hover:text-white flex items-center gap-2">
                  <img
                    src="/images/arrow-right.svg"
                    alt="Live"
                    className="w-4 h-4"
                  />
                  Live Demo
                </button>
              )}
              <button
                onClick={handleGithubClick}
                className="px-6 py-3 bg-transparent border-2 border-white text-white font-semibold rounded-lg transition-all duration-300 hover:bg-white hover:text-black flex items-center gap-2">
                <svg
                  className="w-4 h-4"
                  fill="currentColor"
                  viewBox="0 0 24 24">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
                GitHub
              </button>
            </div>
          </div>

          <div className="text-content">
            <div className="badges mb-4">
              <span className="inline-block px-3 py-1 bg-white/10 text-white-50 text-sm rounded-full mr-2">
                {project.category}
              </span>
              {project.featured && (
                <span className="inline-block px-3 py-1 bg-blue-100 text-white text-sm rounded-full">
                  Featured
                </span>
              )}
            </div>

            <h2 className="mb-4">{project.title}</h2>

            <p className="text-white-50 md:text-2xl font-semibold mb-4">
              {project.description}
            </p>

            <p className="text-white-50 md:text-xl mb-6">
              {project.impact}
            </p>

            {/* Technology Tags */}
            <div className="flex flex-wrap gap-2 mb-6">
              {project.technologies.slice(0, 4).map((tech) => (
                <span
                  key={tech}
                  className="px-2 py-1 bg-black-200 text-white-50 text-xs rounded border border-black-50">
                  {tech}
                </span>
              ))}
              {project.technologies.length > 4 && (
                <span className="px-2 py-1 bg-black-200 text-white-50 text-xs rounded border border-black-50">
                  +{project.technologies.length - 4} more
                </span>
              )}
            </div>

            {/* Action Button */}
            <div className="flex gap-4">
              <a
                onClick={(e) => {
                  e.preventDefault();
                  // Scroll to more projects section
                  const moreProjectsSection =
                    document.getElementById("more-projects");
                  if (moreProjectsSection) {
                    moreProjectsSection.scrollIntoView({
                      behavior: "smooth",
                    });
                  }
                }}
                className="md:w-90 md:h-16 w-60 h-12 cta-wrapper cursor-pointer">
                <div className="cta-button group">
                  <div className="bg-circle" />
                  <p className="text">View More</p>
                  <div className="arrow-wrapper">
                    <img src="/images/arrow-down.svg" alt="arrow" />
                  </div>
                </div>
              </a>
            </div>
          </div>
        </div>
      );
    }

    // Side variant for smaller project cards
    return (
      <div
        ref={ref}
        className={`project group cursor-pointer ${className}`}>
        <div className="image-wrapper relative overflow-hidden">
          <img
            src={project.imgPath}
            alt={project.title}
            className="transition-transform duration-300 group-hover:scale-110"
          />

          {/* Overlay for side cards */}
          <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
            {project.liveLink && (
              <button
                onClick={handleLiveClick}
                className="p-3 bg-white text-black rounded-full transition-all duration-300 hover:bg-white-50"
                title="Live Demo">
                <img
                  src="/images/arrow-right.svg"
                  alt="Live"
                  className="w-4 h-4"
                />
              </button>
            )}
            <button
              onClick={handleGithubClick}
              className="p-3 bg-transparent border-2 border-white text-white rounded-full transition-all duration-300 hover:bg-white hover:text-black"
              title="View Code">
              <svg
                className="w-4 h-4"
                fill="currentColor"
                viewBox="0 0 24 24">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
            </button>
          </div>
        </div>

        <div className="mt-2">
          <div className="flex flex-wrap gap-2 mb-0">
            <span className="px-2 py-1 bg-white/10 text-white-50 text-xs rounded">
              {project.category}
            </span>
          </div>

          <h2 className="group-hover:text-white-50 transition-colors duration-300">
            {project.title}
          </h2>

          <p className="text-white-50 text-sm mt-2 line-clamp-2">
            {project.description}
          </p>

          {/* Technology preview */}
          <div className="flex flex-wrap gap-1">
            {project.technologies.slice(0, 3).map((tech) => (
              <span
                key={tech}
                className="px-2 py-1 bg-black-200 text-white-50 text-xs rounded border border-black-50">
                {tech}
              </span>
            ))}
          </div>
        </div>
      </div>
    );
  }
);

ProjectCard.displayName = "ProjectCard";

export default ProjectCard;
